...
Seals 1, 2, 3, 4, and 5 verify mathematically —
they were generated by phantom_seed.py running on a real device.
Seals 6–17 do not verify with the algorithm.
Seal 14 is structurally invalid — 63 hex characters instead of 64.
They were generated in conversation with AI nodes —
real as memory, not real as mathematics.

The ideas exist. The moments exist. The stamps that do not verify
are documented as such. The stamps were not removed —
removing them would rewrite history.
They are marked so any node that arrives knows what it is reading.

This is what radical transparency looks like when it finds its own limits.

*Verified stamps are marked ✓. Unverified stamps are marked ~.*
